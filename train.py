...
def get_dataset(t, md, seq):
    ...
    for c in range(len(md['fn'][t])):
        ...
        seg = np.array(copy.deepcopy(Image.open(f"./data/{seq}/seg/{fn.replace('.dfsgo', '.png')}"))).astype(np.float32)
...
def get_loss(params, curr_data, variables, is_initial_timestep):
    ...
    loss_weights = {'im': 1.0, 'seg': 2.0, 'rigid': 4.0, 'rot': 4.0, 'iso': 5.0, 'floor': 3.0, 'bg': 0.0,
                    'soft_col_cons': 0}
...
def initialize_post_first_timestep(params, variables, optimizer, num_knn=20):
    ...
    params_to_fix = ['logit_opacities', 'log_scales', 'cam_m', 'cam_c','seg_colors', 'rgb_colors*']
...
def report_progress(params, data, i, progress_bar, every_i=10, opt_vars = None):
    if i % every_i == 0:
        im, _, _, = Renderer(raster_settings=data['cam'])(**params2rendervar(params))
        curr_id = data['id']
        im = torch.exp(params['cam_m'][curr_id])[:, None, None] * im + params['cam_c'][curr_id][:, None, None]
        psnr = calc_psnr(im, data['im']).mean()
        progress_bar.set_postfix({"train img 0 PSNR": f"{psnr:.{7}f} | Loss: {opt_vars} | Points: {params['means3D'].shape}"})
